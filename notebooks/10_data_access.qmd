...
MATL <- pa_md_mat_get(.dataset_name = "material")   # MARA/0MATERIAL
MATS <- pa_md_mat_get(.dataset_name = "mat_sales")  # MVKE/0MAT_SALES
MATP <- pa_md_mat_get(.dataset_name = "mat_plant")  # MARC/0MAT_PLANT

PATH_MD <- pa_ds_stageing_path_get(.staging = "silver", .functional_area = "master_data", "material")

MBTL <- readRDS(file.path(PATH_MD, "B4_MATL.rds"))
MBTS <- readRDS(file.path(PATH_MD, "B4_MATS.rds"))
MBTP <- readRDS(file.path(PATH_MD, "B4_MATP.rds")) 
MBST <- readRDS(file.path(PATH_MD, "B4_MAST.rds")) 
```
...
system.time({
  ACC <-                              # 
    pa_td_dyn_get(
      ...
      .cm_min      = '202401'  , # minimal Cal Month
