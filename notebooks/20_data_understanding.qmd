...
```{r}
SC0 <- 
  ACC[STEP == -1 & VTYPE == '010' & FTYPE == 1 & SALESORG == SORG,  
      .(MATERIAL, Q)] 

SC1 <- 
  copy(SC0)                              %>%
  .[, (tot = sum(Q)), by = .(MATERIAL)] %T>%
  fOpen_as_xlsx()
...
# focus on FR30
Q0 <- sum(ACC[STEP == -1 & VTYPE == '010' & FTYPE == 1 & SALESORG == SORG,  Q])
...
PATH_GLD_SLS <- "C:\\PW\\OneDrive\\ET\\pythia\\data\\test\\Gold\\master_data"
...
fwrite(
  unique(dtACC7[, .(MATERIAL)]),
  file = file.path(PATH_GLD_SLS, "SCP_182.csv")
)
