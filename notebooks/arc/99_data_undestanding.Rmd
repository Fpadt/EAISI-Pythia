...
ggplot(ACTS, aes(x = CALMONTH, y = ACTS)) +
  geom_col(colour = "white", fill = pa_pa_ET_COLSS["CG"]) +
  labs(
    x = "Month",
    y = "Quantity EA", 
...
PRM10023 <- 
  PROMONAT                                                 %>%
  .[Réf == pa_matn1_output(MAT)]                                       %T>%
  fOpen_as_xlsx()
  # fwrite(file.path(PDAT, pa_matn1_output(MAT), "PROMONAT.csv"))
```
...
for (i in 1:length(lst)) {
  wsh <- lst[i] %>% names()
  dt  <- lst[[i]]
  addWorksheet(  wb = wb, sheet = wsh, gridLines = FALSE, tabColour = pa_pa_ET_COLSS["CG"])
