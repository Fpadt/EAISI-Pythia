...
source("C:/RW/EAISI-Padt/R/pa_init.R")
...
source("C:/RW/EAISI-Padt/R/05_data_ingestion.R")
...
```{r}
shell.exec(normalizePath(file.path(".", "config","IB_OB_FLD.xlsx")))
```
...
fTransform_csv_to_parquet(
  ...
  # file_pattern = "^SDSFR(LV|PR[24]_202).*\\.csv$",  # (412|501)
  file_pattern = "^SDSFR(LV).*\\.csv$",  # (412|501)  
  file_spec    = FILE_SPEC,
  ohdest       = "OH_FRPR1",
  verbose      = TRUE
)
...
fTransform_csv_to_parquet(
  source_path  = file.path(PS01, SYS, "DYN"),
  output_path  = file.path(PS02, SYS, "DYN"),
  file_pattern = "^SDSFRPR[13]_202[45].*\\.csv$",   
  file_spec    = FILE_SPEC,
  ohdest       = "OH_FRPR3",
  verbose      = FALSE
)
