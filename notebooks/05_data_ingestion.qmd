...
STK <- 
  ...
  setnames(
    old = STK_FLDNMS$KEY,
    new = STK_FLDNMS$FIELDNM
  )                                                                  %>%
  .[, `:=` (
    CALDAY = ymd(CALDAY),
    END    = fSignLeft(END), # End Stock Qty 
    CNS    = fSignLeft(CNS), # Qty Consignment Stock
    RCP    = fSignLeft(RCP), # Qty Receipt into Total Stock 
    ISS    = fSignLeft(ISS), # Qty Issued from Total Stock
    UNR    = fSignLeft(UNR), # Qty Unrestricted Stock
    SBC    = fSignLeft(SBC), # Qty Subcontracting Stock
    QLI    = fSignLeft(QLI), # Qty Qual. Insp. Stock
    TRN    = fSignLeft(TRN), # Qty Transit Stock
    GRB    = fSignLeft(GRB), # Qty GR Blocked Stock
    BLK    = fSignLeft(BLK), # Qty Blocked Stock
    XI1    = fSignLeft(XI1), # External Quantity Issued from Valuated Stock (1)
    XI2    = fSignLeft(XI2)  # External Quantity Issued from Valuated Stock (2)
  )]                                                                 %T>%
  write_parquet(
    sink = file.path(
      PS02, SYS, "STK", "ARR",
