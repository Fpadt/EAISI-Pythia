...
process_data <- function(yr, input_base_path, output_base_path, sys_value) {
  # Define file paths
  input_csv_file <- file.path(input_base_path, sys_value, "RTP", "CSV", paste0("DD_SALES_QTY_20", yr, ".CSV"))
  output_parquet_file <- file.path(output_base_path, sys_value, "RTP", "ARR", paste0("DD_SALES_QTY_20", yr, ".parquet"))
  ...
  sql_write_data <- write_data_to_parquet(sql_transform_data, output_parquet_file)
  
  # Execute the SQL statements
  tryCatch({
    system.time({
      dbExecute(con, sql_write_data)
      message(glue("Data successfully written to {output_parquet_file}"))
    })
  }, error = function(e) {
...
```{r}
yr <- 21                    # Year parameter

# Run the main function
process_data(yr, PS01, PS02, SYS)
```
