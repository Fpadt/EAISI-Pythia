    ...
    dbGetQuery(
      ...
      paste0(
        "SELECT 
          ...
          ISLS.*,
          DATE_TRUNC('month', CALDAY) AS CALMONTH,
          SLS_QT_SO + SLS_QT_FOC + SLS_QT_DIR as DEMND_QTY
         FROM 
          read_parquet('", FN_ISLS, "') AS ISLS
         INNER JOIN 
          read_parquet('", FN_MATL, "') AS MATL
...
IPH1 <- 
  IALL[PRDH1 %chin% c('07', '08', '10', '15', '53', '64', '65'),
       .(DEMND_QTY = sum(DEMND_QTY)),
         by = .(PRDH1_TMD, SALESORG, CALMONTH)
  ]

tsSLS <- 
  IPH1[, CALMONTH:= yearmonth(CALMONTH)]                             %>%
  as_tsibble(, index = "CALMONTH", 
             key= c("PRDH1_TMD", "SALESORG")) 
...
IPH2 <- 
  IALL[PRDH1 %chin% c('08'),
       .(DEMND_QTY = sum(DEMND_QTY)),
         by = .(PRDH2_TMD, SALESORG, CALMONTH)
  ]
...
IPH3 <- 
  IALL[PRDH2_TMD %like% "Bread",
       .(DEMND_QTY = sum(DEMND_QTY)),
         by = .(PRDH3_TMD, SALESORG, CALMONTH)
  ]
