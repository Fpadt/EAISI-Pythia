...
```{r}
#| label: 'All Sales Data',
#| eval:   false

FN_ISLS <- 
  file.path(
      PS02, SYS, "RTP", "ARR", 
      "DD_HISTO_QTY_21-24.parquet"
    ) %>% 
  normalizePath() 
...
DD_SALES_QTY_2021_INCL_FR50.CSV
...
fSL <- function(phl, IPH){
  ...
  IPH %>%
    ...
    theme(
      ...
      strip.text   = element_text(size = 16)  # Adjust size as needed
    )
...
IPH <- 
  IALL[PRDH1 %chin% ph1,
       .(DEMND_QTY = sum(DEMND_QTY)/1000000),
         by = .(label = PRDH1_TMD, SALESORG, CALMONTH)
  ]
